...
def actor(agent: SACAgent, data_store, env, sampling_rng):
    ...
    current_pos = info["obs"]["tcp_pos"].copy()
    ...
    # env.controller.set_admittance_params(mass=np.array([10.0, 10.0, 10.0, 10.0, 10.0, 10.0]), 
    #                                     damping=np.array([400.0, 350.0, 100.0, 100.0, 100.0, 100.0]), 
    #                                     stiffness=np.array([4000.0, 3000.0, 90.0, 90.0, 90.0, 90.0]))
    
    # env.controller.set_admittance_params(mass=np.array([10.0, 10.0, 10.0, 10.0, 10.0, 10.0]), 
    #                                     damping=np.array([150.0, 100.0, 100.0, 100.0, 100.0, 100.0]), 
    #                                     stiffness=np.array([400.0, 300.0, 90.0, 90.0, 90.0, 90.0]))
    
    # env.controller.set_admittance_params(mass=np.array([2.0, 2.0, 1.0, 1.0, 1.0, 1.0]), 
    #                                     damping=np.array([120.0, 100.0, 20.0, 5.0, 5.0, 5.0]), 
    #                                     stiffness=np.array([4000.0, 3000.0, 150.0, 9.0, 9.0, 9.0]))
    # For sim
    env.controller.set_admittance_params(mass=np.array([2.0, 2.0, 1.0, 1.0, 1.0, 1.0]), 
                                        damping=np.array([100.0, 140.0, 50.0, 5.0, 5.0, 5.0]), 
                                        stiffness=np.array([4000.0, 4000.0, 2000.0, 9.0, 9.0, 9.0]))

    # env.controller.set_admittance_params(mass=np.array([20.0, 20.0, 10.0, 1.0, 1.0, 1.0]), 
    #                                         damping=np.array([350.0, 300.0, 150.0, 5.0, 5.0, 5.0]), 
    ...
    for step in tqdm.tqdm(range(FLAGS.max_steps*10), dynamic_ncols=True):
        ...
        with timer.context("sample_actions"):
            if step < FLAGS.random_steps and FLAGS.loaded_model is None:
                ...
                actions = np.asarray(jax.device_get(actions)) * 0.005
            ...
            # desired_force[:2] = -30 * actions[:2].copy()
            # desired_force[2] = -30 * actions[2].copy()
            # desired_force[:3] =  (pos_ori - pos_ref)
            # env.controller.set_desired_force(desired_force)

            # action_input = actions[:2].copy() * 0.01
            target_pos = actions + current_pos # Here in this place, we assume that the target pose could be followed perfectly, 
            current_pos = target_pos # I've tested the adm_pose and tcp_pose, turns out that adm_pose  can be believed, while tcp_pose not, guess it's due to the ik
            # current_pos[1] += 0.01
            pos_ref[-1] -= 0.2
            action_input_pos_quat[:3] = target_pos
            # print("action_input_pos_quat: ", action_input_pos_quat)
        ...
        with timer.context("step_env"):
            ...
            # if wrench[0] > 1 or wrench[1] >1 or wrench[2] > 1: 
            #     env.controller.set_admittance_params(mass=np.array([20.0, 20.0, 10.0, 1.0, 1.0, 1.0]), 
            #                                 damping=np.array([350.0, 300.0, 150.0, 5.0, 5.0, 5.0]), 
            #                                 stiffness=np.array([0.0, 0.0, 0.0, 9.0, 9.0, 9.0])) 
            # else: 
            ...
            #                                 damping=np.array([350.0, 300.0, 150.0, 5.0, 5.0, 5.0]), 
            #                                 stiffness=np.array([60.0, 60.0, 50.0, 9.0, 9.0, 9.0])) 
            # print("action: ", action_input_pos_quat[:3])
...
def main(_):
    ...
    if FLAGS.loaded_model: 
        ...
        poi = checkpoints.restore_checkpoint(
            FLAGS.loaded_model, target=agent.state, step=8000000
        )
