    ...
    def build_robot_scene(
        ...
        if robot_config.mount_name:
            ...
            if mount_path.exists():
                print(f"DEBUG: Processing mount {robot_config.mount_name} with pos {robot_config.mount_pos} [MODIFIED]")
        ...
        if robot_config.use_ft_sensor:
            if isinstance(robot_config.use_ft_sensor, str):
                sensor_name = robot_config.use_ft_sensor
                sensor_config = getattr(robot_config, "SENSOR_CONFIG", {}).get(sensor_name)
                
                if sensor_config:
                    print(f"DEBUG: Adding force sensor {sensor_name}")
                    ...
                    if flange_body is not None:
                        ...
                        model_path = self.assets_dir / sensor_config["model_path"]
                        scale = sensor_config.get("scale", [0.001, 0.001, 0.001])
                        # Check if mesh already exists to avoid duplicates
                        mesh_name = f"mesh_{sensor_name}"
                        if asset_root.find(f"./mesh[@name='{mesh_name}']") is None:
                            ET.SubElement(asset_root, "mesh", name=mesh_name, 
                                          file=get_rel_path(model_path), 
                                          scale=f"{scale[0]} {scale[1]} {scale[2]}")
                        
                        # Add sensor body to flange
                        sensor_h = sensor_config.get("height", 0.038)
                        sensor_mass = sensor_config.get("mass", 0.2)
                        
                        # We name the sensor body specifically so we can attach gripper to it
                        target_attachment_name = f"{sensor_name}_body"
                        sensor_body = ET.SubElement(flange_body, "body", name=target_attachment_name, pos="0 0 0")
                        
                        ET.SubElement(sensor_body, "geom", type="mesh", mesh=mesh_name, 
                                      mass=str(sensor_mass), material="black" if "black" not in sensor_name else None) # simple material fallback
                        
                        # Add site for sensor readings and gripper attachment
                        # Gripper will attach to the body, but sensor needs a site
                        # The gripper will be attached at 0 0 0 of the sensor body? 
                        # No, gripper should be attached at the end of sensor.
                        # So we need to offset the gripper.
                        # Since we attach gripper to sensor_body, and sensor_body is at 0 0 0 of flange.
                        # The gripper will be at 0 0 0 of sensor_body.
                        # So we need to put the sensor geometry at 0 0 0, but the gripper needs to be at 0 0 height?
                        # Wait, if we attach gripper to sensor_body, the gripper will be at sensor_body's origin.
                        # If the sensor has thickness, the gripper should be shifted by sensor_h.
                        # But `xml_splice` attaches gripper content to `attachment_body`.
                        # It doesn't seemingly add an offset wrapper unless we do it here.
                        # So we should create a specific body for gripper attachment at the offset.
                        
                        gripper_mount_body_name = f"{sensor_name}_mount"
                        gripper_mount_body = ET.SubElement(sensor_body, "body", name=gripper_mount_body_name, pos=f"0 0 {sensor_h}")
                        
                        target_attachment_name = gripper_mount_body_name
                        
                        # Add site at the mount point
                        ET.SubElement(gripper_mount_body, "site", name="ft_frame", pos="0 0 0", size="0.001", rgba="1 0 0 1", type="sphere")
                        
                        # Add sensor definition
                        sensor_root = root.find("sensor")
                        if sensor_root is None:
                            sensor_root = ET.SubElement(root, "sensor")
                        ET.SubElement(sensor_root, "force", name="force_ee", site="ft_frame")
                        ET.SubElement(sensor_root, "torque", name="torque_ee", site="ft_frame")

            elif isinstance(robot_config.use_ft_sensor, bool):
                 # Legacy support
